...
import { Typography, Table, Button, Spin, message, Popconfirm, InputNumber } from "antd";
...
const Flyer = () => {
    ...
    useEffect(() => {
        const fetchFlyers = async () => {
            ...
            try {
                const response = await retryRequest(() => axiosInstance.get("/admin/flyers/all-flyers"));
        ...
        try {
            ...
            setFlyers((prevFlyers) =>
                prevFlyers.map((flyer) =>
                    flyer.FlyerId === flyerId ? { ...flyer, DisplayOrder: newDisplayOrder } : flyer
                )
            );
    ...
    const handleDelete = async (flyerId) => {
        try {
            ...
            setFlyers((prevFlyers) => prevFlyers.filter((flyer) => flyer.FlyerId !== flyerId));
        } catch (error) {
            ...
            render: (text, record) => (
                <div style={{ display: "flex", gap: "10px" }}>
                    ...
                    {editingFlyerId === record.FlyerId ? (
                        <>
                            <Button type="link" onClick={() => handleSaveDisplayOrder(record.FlyerId)}>
                                Save
                            </Button>
                        ...
                        <Button type="link" onClick={() => setEditingFlyerId(record.FlyerId)}>
                            Edit
                        </Button>
                    )}
